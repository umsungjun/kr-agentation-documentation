...
export function HeroDemo() {
  ...
  const feedbackText1 = '기본 스타일로 변경';
  const feedbackText2 = '호버 상태 추가';
  const feedbackText3 = '더 눈에 띄게 만들어주세요';

  // Different terminal output for mobile (metrics instead of sidebar)
  const getTerminalOutput = (mobile: boolean) =>
    mobile
      ? `## 페이지 피드백
    ...
    const runAnimation = async () => {
      ...
      if (!isMobile) {
        ...
        for (let i = 1; i <= steps; i++) {
          ...
          if (progress > 0.25)
            setSelectedSidebarIcons((s) => (s.includes(2) ? s : [...s, 2]));
          if (progress > 0.5)
            setSelectedSidebarIcons((s) => (s.includes(1) ? s : [...s, 1]));
          if (progress > 0.75)
            setSelectedSidebarIcons((s) => (s.includes(0) ? s : [...s, 0]));
          await delay(25);
        }
        if (cancelled) return;
        await delay(200);
        ...
        for (let i = 1; i <= steps; i++) {
          ...
          if (progress > 0.2)
            setSelectedMetrics((s) => (s.includes(0) ? s : [...s, 0]));
          if (progress > 0.5)
            setSelectedMetrics((s) => (s.includes(1) ? s : [...s, 1]));
          if (progress > 0.8)
            setSelectedMetrics((s) => (s.includes(2) ? s : [...s, 2]));
          await delay(25);
        }
        if (cancelled) return;
        await delay(150);
        ...
        setPopupHeader('메트릭 카드');
        setTypedText('');
        // Position popup below the metrics selection
        const containerWidth = contentRef.current?.offsetWidth || 300;
        setPopupPos({
          x: Math.min(
            metricsPos.x + metricsPos.width / 2,
            containerWidth - 100,
          ),
          y: dragEndY + 7,
        });
        ...
        const mobileDragFeedback = '호버 상태 추가';
        ...
        setGreenMarkerPos({
          x: dragEndX,
          y: metricsPos.y + metricsPos.height / 2,
        });
        setShowGreenMarker(true);
        await delay(300);
        if (cancelled) return;
      ...
      const textPopupX = Math.min(
        Math.max(100, (textStartX + textEndX) / 2),
        (contentRef.current?.offsetWidth || 300) - 100,
      );
    ...
    <div className="hero-demo-container">
        ...
        <div className="hero-demo-content" ref={contentRef}>
          ...
          <div className="hero-demo-sidebar">
            ...
            <div className="hero-demo-sidebar-icons" ref={sidebarIconsRef}>
              <div
                className={`hero-demo-sidebar-icon ${selectedSidebarIcons.includes(0) ? 'selected' : ''}`}
              />
              <div
                className={`hero-demo-sidebar-icon active ${selectedSidebarIcons.includes(1) ? 'selected' : ''}`}
              />
              <div
                className={`hero-demo-sidebar-icon ${selectedSidebarIcons.includes(2) ? 'selected' : ''}`}
              />
            </div>
          ...
          <div className="hero-demo-main">
            ...
            <div className="hero-demo-header">
              <div className="hero-demo-header-left">
                ...
                <span className="hero-demo-title-text" ref={timeRef}>
                  {headerText}
                </span>
              </div>
            ...
            <div className="hero-demo-metrics" ref={metricsRef}>
              <div
                className={`hero-demo-metric ${selectedMetrics.includes(0) ? 'selected' : ''}`}
              >
                <div className="hero-demo-metric-label" />
                <div className="hero-demo-metric-value" />
              </div>
              <div
                className={`hero-demo-metric ${selectedMetrics.includes(1) ? 'selected' : ''}`}
              >
                <div className="hero-demo-metric-label" style={{ width: 55 }} />
                <div className="hero-demo-metric-value wide" />
              </div>
              <div
                className={`hero-demo-metric ${selectedMetrics.includes(2) ? 'selected' : ''}`}
              >
            ...
            <div
              ref={toolbarRef}
              className={`hero-demo-toolbar ${isToolbarExpanded ? 'expanded' : ''} ${isToolbarHovered ? 'hovered' : ''} ${isToolbarClicking ? 'clicking' : ''}`}
            >
              ...
              <div className="hero-demo-toolbar-buttons">
                ...
                <div
                  ref={copyBtnRef}
                  className={`hero-demo-toolbar-btn ${copyClicked ? 'active' : ''} ${copyHovered ? 'hovered' : ''}`}
                >
                  {copyClicked ? (
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                      <path
          ...
          {/* Existing markers */}
          <div
            className="hero-demo-marker visible"
          ...
          <div
            className={`hero-demo-marker ${showNewMarker ? 'visible' : ''}`}
            style={{
          ...
          <div
            className={`hero-demo-marker green ${showGreenMarker ? 'visible' : ''}`}
            style={{ top: greenMarkerPos.y, left: greenMarkerPos.x }}
          >
            4
          </div>
          ...
          <div
            className={`hero-demo-marker orange ${showOrangeMarker ? 'visible' : ''}`}
            style={{ top: orangeMarkerPos.y, left: orangeMarkerPos.x }}
          >
            {isMobileView ? 3 : 5}
          </div>
          ...
          {/* Popup */}
          <div
            className={`hero-demo-popup ${showPopup ? 'visible' : ''}`}
          ...
          >
            ...
            <div className="hero-demo-popup-actions">
              <div className="hero-demo-popup-btn cancel">취소</div>
              <div
                className={`hero-demo-popup-btn submit ${popupHeader === '사이드바 아이콘' ? 'green' : ''} ${popupHeader === '메트릭 카드' ? 'green' : ''} ${popupHeader.startsWith('"') ? 'orange' : ''}`}
              >
                추가
              </div>
            </div>
          ...
          >
            <div
              className={`hero-demo-cursor-pointer ${isCrosshair || isIBeam ? 'hidden' : ''}`}
            >
              <svg
                height="24"
                width="24"
            ...
            <div
              className={`hero-demo-cursor-crosshair ${isCrosshair && !isIBeam ? '' : 'hidden'}`}
            >
            ...
            <div
              className={`hero-demo-cursor-ibeam ${isIBeam ? '' : 'hidden'}`}
            >
      ...
      <div className={`hero-demo-terminal ${showTerminal ? 'visible' : ''}`}>
        <div className="hero-demo-terminal-bar">
          ...
          <div className="hero-demo-terminal-title">Benji의 프로젝트</div>
        </div>
          ...
          <div className="hero-demo-terminal-welcome">
            ...
            >
              ...
              <g transform="translate(56, 26) scale(0.35)">
                ...
                <path
                  d="M34.998 16.1953H20.998V32.3953H34.998V16.1953Z"
                  fill="#D77757"
                />
                <rect
                  x="35"
                ...
                <path
                  d="M105.002 16.1953H91.002V32.3953H105.002V16.1953Z"
                  fill="#D77757"
                />
                <path d="M119 32.4023H7V48.6023H119V32.4023Z" fill="#D77757" />
                <path
                  d="M104.998 48.5977H20.998V64.7977H104.998V48.5977Z"
                  fill="#D77757"
                />
                ...
                <path
                  d="M98.002 64.8047H91.002V81.0047H98.002V64.8047Z"
                  fill="#D77757"
                />
              </g>
